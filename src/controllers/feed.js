...
import Post, { find, findById, findByIdAndRemove } from "../models/post";
...
export async function getPosts(req, res, next) {
  ...
  const perPage = 2;
  try {
    const totalItems = await find().countDocuments();
...
export async function updatePost(req, res, next) {
  ...
  try {
    const post = await findById(postId).populate("creator");
    if (!post) {
      const error = new Error("Could not find post.");
      error.statusCode = 404;
      throw error;
